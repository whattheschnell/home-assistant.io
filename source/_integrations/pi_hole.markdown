...
verify_ssl:
  ...
  default: true
api_key:
  description: API Key for interacting with the Pi-hole. This is not required if you want to just query the Pi-hole for usage statistics.  API Key may need to be set to null ( "" ) if using the Pi Hole addon for HASSIO 
  required: false
  type: string
  default: None
{% endconfiguration %}
